...
$db = new Database();
$result = $db->select(array("user_id", "Event_Title", "quantity"), "", "bookmarks");
            ...
            for ($i = $startIndex; $i < $startIndex + 10 && $i < count($resultArr); $i++) {
                echo <<<HTML
                    <tr class="text-center">
                        ...
                        <td>{$resultArr[$i][2]}</td>
                        <td>
                            <button onclick="toRedirect({$resultArr[$i][0]}, 'edit_bookmark.php')" class="btn btn-primary">Edit</button>
