...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['pageNo'])) {
    if ($_GET['pageNo'] >= 1 && $_GET['pageNo'] <= $pageCount) {
        ...
        header("Location: $sevRoot/admin/users/list_bookmarks.php");
        die();
    }
...
<head>
    ...
    <script>
        function toRedirect(userId, eventTitle, location) {
            window.location = `${location}?user_id=${userId}&event_Title_FK=${eventTitle};`;
        }
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                        ...
                        <input id="quantityInput" name="quantity" type="hidden" value="">
            ...
            for ($i = $startIndex; $i < $startIndex + 10 && $i < count($resultArr); $i++) {
                echo <<<HTML
                    <tr class="text-center">
                        ...
                        <td>{$resultArr[$i][4]}</td>
                        <td>
                            <button onclick="toRedirect({$resultArr[$i][0]}, '{$resultArr[$i][1]}', 'edit_participant.php')" class="btn btn-primary">Edit</button>
